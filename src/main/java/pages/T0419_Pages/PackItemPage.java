...
public class PackItemPage {
	/**
	 * Constructor
	 */
	public PackItemPage(WebDriver driver) {
	...
	public void AddProduct(String UserName) {
		try {
			...
			String Updload_Path = System.getProperty("user.dir") + File.separator + "src" + File.separator + "test"
			...
			BaseTest.getDriver().switchTo().defaultContent();
			BaseTest.getDriver().switchTo().frame("MultiPageiframeDlg");
			Utilities.SendKeys(BaseTest.getDriver(), FromDept, "89");

			// Select Indent Department
			BaseTest.getDriver().switchTo().defaultContent();
			BaseTest.getDriver().switchTo().frame("MultiPageiframeDlg");
			Utilities.Click(BaseTest.getDriver(), IndentDept);
			Utilities.SendKeys(BaseTest.getDriver(), IndentDept, "1257");
			...
			Utilities.SendKeys(BaseTest.getDriver(), getItemSearchElement(0), "%%");
			getItemSearchElement(0).sendKeys(Keys.TAB);
			BaseTest.getDriver().switchTo().defaultContent();
			BaseTest.getDriver().switchTo().frame("iframeGridDialog");
			Utilities.WaitTillElementDisplayed(BaseTest.getDriver(), ItemFilter);
			String text = Item.get(0).getText();

			Utilities.SendKeys(BaseTest.getDriver(), ItemFilter, "TBCG0037");
			DynamicWait.smallWait();
			Utilities.Click(BaseTest.getDriver(), FirstColumn);
			BaseTest.getDriver().switchTo().defaultContent();
			BaseTest.getDriver().switchTo().frame("MultiPageiframeDlg");
			...
			ExtentTestManager.createAssertTestStepWithScreenshot("AddProduct", Status.FAIL,
					"Exception found in Method - AddProduct", true, e);
		}
	}
