...
public class MasterReq_Page {
    /**
     * Constructor
     */
    public MasterReq_Page(WebDriver driver) {
...
//            }
            ...
            for (int i = 0; i < 6; i++) {
                ...
                while (usedTexts.contains(text)) {
                    int randomIndex1 = random.nextInt(Item.size() - 1);
            ...
            BaseTest.getDriver().switchTo().defaultContent();
            BaseTest.getDriver().switchTo().frame("MultiPageiframeDlg");
            Utilities.WaitTillElementIsClickable(BaseTest.getDriver(), NewBtn);
