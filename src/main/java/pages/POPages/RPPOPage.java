...
public class RPPOPage {
    /**
     * Constructor
     */
    public RPPOPage(WebDriver driver) {
    ...
    private WebElement Search_vendor_click;
     ...
     **/
    public void POCreate(String UserName) {
        try {
            ...
            String Value="CCL001";
            //Select Vendor
            Utilities.SendKeys(BaseTest.getDriver(),Search_vendor,"%%");
            Utilities.Click(BaseTest.getDriver(),Search_vendor_click);

            BaseTest.getDriver().switchTo().defaultContent();
            BaseTest.getDriver().switchTo().frame("iframeGridDialog");

            Utilities.SendKeys(BaseTest.getDriver(),POType,Value);
