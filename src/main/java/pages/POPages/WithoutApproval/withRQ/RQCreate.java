...
public class RQCreate {
    ...
    @FindBy(xpath = "//*[starts-with(@id, 'item_id_1_')]")
    private List<WebElement> item;
...
//            Utilities.Click(BaseTest.getDriver(), OkButton);
    ...
    public void validate(){
        try {
            ...
            System.out.println("Items in PO are :"+ POitems);




            System.out.println("POitems: " + POitems);
            System.out.println("items: " + items);

            // Compare lists as sets
            boolean areSame = new HashSet<>(POitems).equals(new HashSet<>(items));
            if (areSame){
                ExtentTestManager.createAssertTestStepWithScreenshot("Item Validation",Status.PASS,"RQ items and PO items are same",true);
            }
            System.out.println("Do lists contain the same items (ignoring order)? " + areSame);

            // Print differences if lists are not the same
            if (!areSame) {
                List<String> onlyInPOitems = new ArrayList<>(POitems);
                onlyInPOitems.removeAll(items);
                List<String> onlyInItems = new ArrayList<>(items);
                onlyInItems.removeAll(POitems);
                System.out.println("Items only in POitems: " + onlyInPOitems);
                System.out.println("Items only in items: " + onlyInItems);
            }
