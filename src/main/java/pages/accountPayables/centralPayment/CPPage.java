package pages.accountPayables.centralPayment;
...
public class CPPage {
    ...
    @FindBy(id = "fx_bill_amount_passed_4_0")
    ...
    @FindBy(id = "cphBody_txtpayment_due_dt")
    ...
    public void addPJVEntry() {
        ...
        try {
            ...
            Utilities.SendKeys(supplierField, "AP CP");
...
//            uncomment if you want to chose the random supplier
            ...
            // Enter amount
            toDlg();
            Utilities.WaitTillElementClickable(supplierBillAmountField);
            ...
            Utilities.Click(date1);
            String text = date1.getAttribute("value");
            Utilities.SendKeys(dueDate, text);
            // Select account
            toDtl();
...
//            );
    ...
    @FindBy(xpath = "//a[text()='Payment Request']")
    private WebElement paymentRequestLink;

    @FindBy(xpath = "//input[@id='cphBody_rdoOtherUnit']")
    private WebElement otherUnitRadio;

    @FindBy(id = "cphBody_txtunit_id")
    private WebElement unitField;

    @FindBy(id = "cphBody_txtVendorFrom")
    private WebElement vendorFromField;

    @FindBy(id = "cphBody_txtVendorTo")
    private WebElement vendorToField;

    @FindBy(id = "cphBody_btnSearch")
    private WebElement searchButton;

    @FindBy(xpath = "//input[contains(@id,'process_9_0')]")
    private WebElement processCheckbox;

    @FindBy(xpath = "//input[contains(@id,'req_remarks_20_0')]")
    ...
    public void performPaymentRequest() {
        try {
            ...
            Utilities.Click(BaseTest.getDriver(), accountsPayablesLink);
            Utilities.Click(BaseTest.getDriver(), paymentRequestLink);

            BaseTest.getDriver().switchTo().defaultContent();
            BaseTest.getDriver().switchTo().frame("MultiPageiframeBrw");
            Utilities.Click(BaseTest.getDriver(), otherUnitRadio);
            Utilities.SendKeys(BaseTest.getDriver(), unitField, "BHOTL");
            Utilities.SendKeys(BaseTest.getDriver(), vendorFromField, "AP CP");
            Utilities.SendKeys(BaseTest.getDriver(), vendorToField, "AP CP");
            Utilities.Click(BaseTest.getDriver(), searchButton);

            Utilities.Click(BaseTest.getDriver(), processCheckbox);
            Utilities.SendKeys(BaseTest.getDriver(), remarksField, "AP CP");
            Utilities.Click(BaseTest.getDriver(), saveButton);
            BaseTest.getDriver().switchTo().defaultContent();
            BaseTest.getDriver().switchTo().frame("iframeGridDialog");
            Utilities.Click(BaseTest.getDriver(), okButton);

            ExtentTestManager.createAssertTestStepWithScreenshot("Payment Request", Status.PASS, "Payment Request performed successfully", true);
        } catch (Exception e) {
            ExtentTestManager.createAssertTestStepWithScreenshot("Payment Request", Status.FAIL, "Failed to perform Payment Request", true, e);
        }
    }


    @FindBy(xpath = "//a[text()='Process Payment']")
    private WebElement processPaymentLink;

    @FindBy(id = "cphHeader_ddlPropertyList")
    private WebElement propertyDropdown;



    @FindBy(id = "cphBody_txtStartChqNo")
    private WebElement startChequeField;

    @FindBy(id = "cphBody_txtEndChqNo")
    private WebElement endChequeField;
    ...
    public void processPayment() {
        try {
            ...
            Utilities.selectBy(BaseTest.getDriver(), "value", propertyDropdown, "BHOTL");
            try {
            ...
            BaseTest.getDriver().switchTo().frame("MultiPageiframeBrw");
            Utilities.Click(BaseTest.getDriver(), accountsPayablesLink);
            Utilities.Click(BaseTest.getDriver(), processPaymentLink);
            ...
            String chequeNO = startChequeField.getAttribute("value");
            Utilities.SendKeys(BaseTest.getDriver(), endChequeField, chequeNO);
            ...
            ExtentTestManager.createAssertTestStepWithScreenshot("Process Payment", Status.PASS, "Process Payment completed successfully", true);
        } catch (Exception e) {
            ExtentTestManager.createAssertTestStepWithScreenshot("Process Payment", Status.FAIL, "Failed to complete Process Payment", true, e);
        }
    }
