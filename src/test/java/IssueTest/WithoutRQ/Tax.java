...
public class Tax extends BaseTest {
    ...
    public void SaveIntoExcel(Method m) {
        ExtentTestManager.startTest(this.getClass().getSimpleName() + " :: " + m.getName(), "SaveIntoExcel");
        try {
            IssueThread.get().SaveDetailsinExcel();
            ExtentTestManager.createAssertTestStepWithScreenshot("SaveIntoExcel", Status.PASS, "SaveIntoExcel Pass", true);
        } catch (Exception e) {
            ExtentTestManager.createAssertTestStepWithScreenshot("SaveIntoExcel", Status.FAIL, "Exception found in SaveIntoExcel", true, e);
        }
    }
