package accountpayables.settlement;
...
import pages.accountPayables.settlement.SettlePage;
 ...
 * Date: 23-01-2025
 */
public class SettlePJVCreditNotesTest extends BaseTest {

    private final ThreadLocal<Login> LoginThread = new ThreadLocal<>();
    private final ThreadLocal<PO> po = new ThreadLocal<>();
    private final ThreadLocal<SettlePage> settle = new ThreadLocal<>();
    ...
    public void setUpBrowser() {
        ...
        settle.set(new SettlePage(getDriver()));
    ...
    public void POCreate(Method m) {
        ...
        try {
            ...
            settle.get().accountPayable();

            settle.get().settlePjvAndCreditNotesFlow();
