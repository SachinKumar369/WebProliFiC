 ...
 * Date: 26-01-2025
 */
public class MultipleInvoiceTest extends BaseTest {
    ...
    @Test(priority = 2)
    public void MultipleInvoice_(Method m) {
        ExtentTestManager.startTest(this.getClass().getSimpleName() + " :: " + m.getName(), " MultipleInvoice");

        try {
            makePJV.get().accounting();
            multipleInv.get().completePJVPickerFlow();

            ExtentTestManager.createAssertTestStepWithScreenshot("MultipleInvoice", Status.PASS, "MultipleInvoice Creation Pass", true);

        } catch (Exception e) {
            ExtentTestManager.createAssertTestStepWithScreenshot("MultipleInvoice", Status.FAIL, "Exception found in MultipleInvoice", true, e);
        }
    }
