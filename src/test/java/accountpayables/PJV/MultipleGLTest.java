 ...
 * Date: 26-01-2025
 */
public class MultipleGLTest extends BaseTest {
    ...
    @Test(priority = 2)
    public void MultipleGL(Method m) {
        ExtentTestManager.startTest(this.getClass().getSimpleName() + " :: " + m.getName(), "MultipleGL");

        try {
            makePJV.get().accounting();
            multipleInv.get().multipleGLFlow();

            ExtentTestManager.createAssertTestStepWithScreenshot("MultipleGL", Status.PASS, "MultipleGL Pass", true);

        } catch (Exception e) {
            ExtentTestManager.createAssertTestStepWithScreenshot("MultipleGL", Status.FAIL, "Exception found in MultipleGL", true, e);
        }
    }
