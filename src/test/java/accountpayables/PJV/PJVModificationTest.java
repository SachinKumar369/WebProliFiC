 ...
 * Date: 26-01-2025
 */
public class PJVModificationTest extends BaseTest {
    ...
    @Test(priority = 2)
    public void PJV_Modification(Method m) {
        ExtentTestManager.startTest(this.getClass().getSimpleName() + " :: " + m.getName(), "PJV_Modification");

        try {
            makePJV.get().accounting();
            multipleInv.get().completePJVPickerFlow();
            multipleInv.get().modifyPJV();
            ExtentTestManager.createAssertTestStepWithScreenshot("PJV_Modification", Status.PASS, "PJV_Modification Pass", true);

        } catch (Exception e) {
            ExtentTestManager.createAssertTestStepWithScreenshot("PJV_Modification", Status.FAIL, "Exception found in PJV_Modification", true, e);
        }
    }
